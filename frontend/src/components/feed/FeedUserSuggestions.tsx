...
export function FeedUserSuggestions({ users }: FeedUserSuggestionsProps) {
  ...
  return (
    <Box sx={{ bgcolor: theme.palette.background.paper, border: `1px solid ${theme.palette.divider}`, borderRadius: 2, p: 2, transition: 'box-shadow .2s ease, border-color .2s ease', '&:hover': { borderColor: theme.palette.primary.main, boxShadow: '0 4px 20px rgba(0,0,0,.3)' } }}>
      ...
      {users.map((user) => (
        <Box key={user.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 0.75, px: 0.5, mx: -0.5, borderRadius: 1, transition: 'background .2s ease', '&:hover': { bgcolor: theme.custom.surface2 } }}>
          <UserLink username={user.username} displayName={user.displayName} avatarUrl={user.userAvatarUrl} size="md" withAvatar subtitle={<>{user.commonFollowers} общих подписок</>} />
          <IconButton size="small" disabled={loading} onClick={(e) => { e.stopPropagation(); handleToggleFollow(user); }} sx={{ color: user.isFollowed ? theme.palette.primary.main : theme.palette.text.secondary, transition: 'color .2s ease, background .2s ease', '&:hover': { color: theme.palette.primary.main, bgcolor: `${theme.palette.primary.main}15` } }}>
            {loading ? <CircularProgress size={16} /> : user.isFollowed ? <PersonRemove sx={{ fontSize: 18 }} /> : <PersonAdd sx={{ fontSize: 18 }} />}
          </IconButton>
        </Box>
      ))}
    </Box>
