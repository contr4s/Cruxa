  ...
  return (
    <>
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
        ...
        {Array.isArray(hours) ? hours.map((entry) => (
          <Typography key={entry.days} sx={{ fontSize: '0.82rem', color: theme.palette.text.secondary, pl: 2.5 }}>
            {entry.days}: {entry.from} – {entry.to}
          </Typography>
        )) : null}
      </Box>
