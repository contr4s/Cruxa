  ...
  return (
    <Box sx={{ flex: 2, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1 }}>
        {gym.area && (
          <StatMiniCard icon={<SpaceDashboard sx={{ fontSize: 16 }} />} label="Площадь" value={`${gym.area} м²`} />
        )}
        {gym.maxHeight && (
          <StatMiniCard icon={<Height sx={{ fontSize: 16 }} />} label="Высота" value={`${gym.maxHeight} м`} />
        )}
        {gym.yearOpened && (
          <StatMiniCard icon={<CalendarMonth sx={{ fontSize: 16 }} />} label="Год открытия" value={String(gym.yearOpened)} />
        )}
      </Box>
    </Box>
  );
