...
- `Rating` (int) *Опционально. Оценка трассы пользователем от 1 до 5 (насколько понравилась)*
- `MediaUrls` (string[]) *Опционально. Фото/видео конкретного пролаза (например, видео попытки)*
- `PrivateNotes` (text) *Опционально. Приватные заметки чисто для себя (например, расклад трассы, 'ключ на второй оттяжке')*
- `PublicReview` (text) *Опционально. Публичный отзыв о трассе (будет виден рутсеттерам и другим скалолазам)*
- `CreatedAt` (timestamp)

### 7. `Follower` (Социальные связи)
- `FollowerId` (FK -> User)
- `FolloweeId` (FK -> User)
- `CreatedAt` (timestamp)

### 8. `Like` & `Comment` (Социальная активность)
